...
private:
  ...
  bool init_can_device()
  {
    ...
    // 设置通道 0 的波特率
    // CAN 1Mbps, CANFD 5Mbps, 采样点 0.75
    device_channel_set_baud_with_sp(dev_, 0, true, 1000000, 5000000, 0.75f, 0.75f);

    // 获取并打印波特率信息
    device_baud_t baud = {0};
    if (device_channel_get_baudrate(dev_, 0, &baud)) {
      RCLCPP_INFO(this->get_logger(), "Channel 0 - CAN baud: %d, CANFD baud: %d", 
  ...
  void on_cmd_vel(const geometry_msgs::msg::Twist::SharedPtr msg)
  {
    ...
    RCLCPP_DEBUG(this->get_logger(), "Sent chassis cmd id=0x%X vx=%.3f vy=%.3f wz=%.3f", 
